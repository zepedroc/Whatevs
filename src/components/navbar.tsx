...
export default function NavBar() {
  ...
  const navItems = [
    ...
    { key: 'euromillions', href: `/${locale}/games/euromillions`, label: t('navigation.euromillions') },
  ];
            ...
            {navItems.map((item) => (
              <NavigationMenuLink key={item.key} asChild>
                <Link
                  ...
                  className="group inline-flex h-9 w-max items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors focus:outline-hidden disabled:pointer-events-none disabled:opacity-50"
                  href={item.href}
                >
                  {item.label}
                </Link>
