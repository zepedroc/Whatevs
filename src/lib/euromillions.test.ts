...
describe('EuroMillions generator', () => {
  ...
  it('validate function catches invalid picks', () => {
    expect(
      validateEuroMillionsPick({ numbers: [0, 2, 3, 4, 5], stars: [1, 2] })
    ).toBe(false);
    expect(
      validateEuroMillionsPick({ numbers: [1, 2, 3, 4, 5], stars: [0, 2] })
    ).toBe(false);
    expect(
      validateEuroMillionsPick({ numbers: [1, 2, 3, 4, 4], stars: [1, 2] })
    ).toBe(false);
  });
});
